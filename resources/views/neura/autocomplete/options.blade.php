...
<neura::popup x-show="open" class="w-full! rounded-lg" x-on:keydown.escape="open = false"
    ...
    <template x-if="filteredOptions.length > 0">
        <ul role="listbox"
            ...
            <template x-for="(option, index) in filteredOptions" :key="index">
                <li :data-option-index="index" :data-value="option.value" :data-label="option.label"
                    x-on:click="select(option)" x-on:mouseenter="activeIndex = index"
                    :class="{
                    ...
                    <div class="flex items-center justify-between gap-2">
                        ...
                        <neura::icon x-show="selectedValue === option.value" name="check"
    ...
    <template x-if="filteredOptions.length === 0 && search.length >= minChars">
        <div class="px-4 py-8 text-center">
            <svg class="mx-auto size-12 text-neutral-300 dark:text-neutral-600 mb-3" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="1.5">
                <path stroke-linecap="round" stroke-linejoin="round"
                    d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
            </svg>
            <p class="text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-1"
                x-text="window.t('noResultsFound')">
            </p>
