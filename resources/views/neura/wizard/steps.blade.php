...
<div wire:ignore wire:key="wizard-steps-{{ $variant }}-{{ $orientation }}"
    {{ $attributes->merge([
        'class' => match (true) {
            $orientation === 'vertical'
                => 'flex flex-col w-full md:w-80 shrink-0 bg-surface-inset border-r border-edge p-8 space-y-6',

            $variant === 'pills'
                => 'inline-flex h-10 w-fit items-center justify-center rounded-md bg-surface-inset p-1 mb-8',
    ...
    role="tablist" aria-orientation="{{ $orientation }}" x-data="{
        ...
        <div class="{{ $orientation === 'vertical' ? 'flex flex-col gap-1 w-full' : 'flex w-full gap-1' }}">
            <template x-for="(step, idx) in steps" :key="idx">
                <button type="button" role="tab" x-on:click="goToStep(idx + 1)"
                    :aria-selected="isStepActive(idx + 1)" :disabled="!canGoToStep(idx + 1)"
    ...
    @elseif($variant === 'tabs')
        <template x-for="(step, idx) in steps" :key="idx">
            <button type="button" role="tab" x-on:click="goToStep(idx + 1)" :aria-selected="isStepActive(idx + 1)"
    ...
    @else
        <template x-for="(step, idx) in steps" :key="idx">
            <div
                :class="[orientation === 'vertical' ? 'flex flex-col relative ' + sizeConfig.minHeight :
                ...
                <div :class="{
                    ...
                    x-on:click="goToStep(idx + 1)" role="tab" :aria-selected="isStepActive(idx + 1)"
                    :aria-current="isStepActive(idx + 1) ? 'step' : null">
                    <div class="relative flex-shrink-0">
                        <div class="flex items-center justify-center transition-all duration-300 rounded-full border"
                            :class="[
                                sizeConfig.circle,
                                isStepActive(idx + 1) && (colorConfig.active + ' border-transparent'),
                                isStepCompleted(idx + 1) && (colorConfig.completed),
                                !isStepActive(idx + 1) && !isStepCompleted(idx + 1) &&
                                'bg-transparent border-neutral-300 dark:border-white/[0.15] text-fg-disabled',
                                canGoToStep(idx + 1) && !isStepActive(idx + 1) && !isStepCompleted(idx + 1) &&
                                'group-hover:border-neutral-400 dark:group-hover:border-white/25 group-hover:text-fg-muted'
                            ].filter(Boolean).join(' ')">
                            <svg x-show="isStepCompleted(idx + 1)" x-cloak :class="sizeConfig.checkIcon" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                            </svg>
                            <span x-show="!isStepCompleted(idx + 1)" x-cloak x-text="idx + 1"
                                class="leading-none font-medium"></span>
                        </div>
                    </div>

                    <div :class="orientation === 'horizontal' ? 'mt-1.5 text-center max-w-[120px]' : 'text-left'">
                        <span class="font-medium transition-all duration-300 block leading-tight"
                            :class="[
                                sizeConfig.label,
                            ...
                            x-text="typeof step === 'object' ? step.label : step"></span>
                        <span x-show="typeof step === 'object' && step.description" x-cloak
                            class="block mt-0.5 font-normal transition-all duration-300 text-fg-muted"
                            :class="sizeConfig.description"
                            x-text="typeof step === 'object' ? step.description : ''"></span>
                    </div>
                </div>

                <div x-show="idx < steps.length - 1" x-cloak class="flex-1"
                    :class="[
                        orientation === 'horizontal' && ('self-start ' + sizeConfig.connectorOffset + ' ' + sizeConfig
                            .gap + ' mx-3'),
                    ...
                    <div class="relative w-full overflow-hidden rounded-full"
                        ...
                        <div class="absolute inset-y-0 left-0 rounded-full transition-all duration-500 ease-out"
                            ...
                            :style="isStepCompleted(idx + 1) ? (orientation === 'vertical' ? 'height:100%' : 'width:100%') : (
