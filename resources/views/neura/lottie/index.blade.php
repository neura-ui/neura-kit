...
@php
    ...
    $dropShadowClass = PackResolver::dropShadow(neura_config('lottie', 'shadow'));
    $svgDropShadowClass = $dropShadowClass === 'drop-shadow-none' ? '' : "[&_svg]:{$dropShadowClass}";
@endphp
...
<div
    ...
    {{ $attributes->merge([
        'class' => "absolute {$position} {$size} {$opacity} pointer-events-none z-0 transition-transform duration-100 ease-in-out {$monochromeClasses} {$svgDropShadowClass} [&_svg]:h-auto [&_svg]:w-auto"
    ]) }}
